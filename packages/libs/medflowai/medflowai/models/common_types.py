...
class UnifiedLLMResponse(BaseModel):
    ...
    # ------------------------------------------------------------------
    # Backwards-compatibility: accept `error_message` in input kwargs and
    # expose a convenience alias property so that downstream code or tests
    # referencing `error_message` continue to work without modification.
    # ------------------------------------------------------------------

    @property
    def error_message(self) -> Optional[str]:
        """Alias for :pyattr:`error` kept for legacy compatibility."""
        return self.error

    @classmethod
    def __get_validators__(cls):
        # Yield the default validators first
        yield from super().__get_validators__()

        # Then a custom validator mapping `error_message` to `error`
        def _map_error_message(values):
            # If `error` is missing but legacy `error_message` provided, copy it
            if values.get("error") is None and (legacy := values.get("error_message")) is not None:
                values["error"] = legacy
            return values
