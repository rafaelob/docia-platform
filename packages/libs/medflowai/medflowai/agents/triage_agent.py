...
# Define a more specific input model if needed, inheriting from GenericInput
class TriageAgentInput(GenericInput):
    # query is already in GenericInput. Add specifics if any.
    # session_id: Optional[str] = Field(None, description="Session ID for context tracking.")
    pass

# Define a more specific output model, inheriting from GenericOutput
class TriageAgentOutput(GenericOutput):
    # response is already in GenericOutput. Add specifics.
    ...
    async def run(self, input_data: TriageAgentInput, context: Optional[Dict[str, Any]] = None) -> TriageAgentOutput:
        ...
        try:
            ...
            try:
                ...
                # Ensure the 'response' field (from GenericOutput) is populated if the LLM doesn't directly set it.
...
if __name__ == "__main__":
    ...
    # Example Usage:
    # import asyncio
    # from medflowai.models.agent_io_models import GenericInput # Assuming path is set up
